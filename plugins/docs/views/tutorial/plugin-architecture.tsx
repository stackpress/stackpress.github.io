...
export function Head(props: ServerPageProps<ServerConfigProps>) {
  ...
  const { _ } = useLanguage();
  //variables
  const title = _('1. Plugin Archiecture - Stackpress Tutorial');
  const description = _(
...
export function Body() {
  ...
  return (
    <article className="px-h-100-0 overflow-auto px-px-20 px-pb-20 px-fs-15">
      ...
      <H1>1. The Plugin Architecture</H1>

      <P>
        The plugin architecture is a file structure pattern designed to 
        separate the project code by unit feature. This allows developers 
        to progressively toggle each one at any given time.
      </P>

      {/*------------------------------------------------------------*/}

      <a id="development-config"></a>
      <H2>1.1. Development Config File</H2>
      ...
      <Nav
        prev={{ text: 'Tutorial', href: '/docs/tutorial' }}
        next={{ text: '2. Server Routes', href: '/docs/tutorial/server-routes' }}
      />
    </article>
  );
}
