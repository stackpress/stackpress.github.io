...
const data = nest()
const data = nest({ foo: { foo: 'bar', bar: 'foo' }, bar: 'foo' })
const data = nest<Record<string, string>>()`,
//1-------------------------------------------------------------------//
];
...
export function Head(props: ServerPageProps<ServerConfigProps>) {
  ...
  const { _ } = useLanguage();
  //variables
  const title = _('Callable Nest - Data - Stackpress Documentation');
  const description = _(
    'A callable map is a type of callable class. In this case it\'s '
    + 'a Nest() class that can be called as a function.'
  );
  return (
    <>
      <title>{title}</title>
  ...
  return (
    <menu className="px-m-0 px-px-10 px-py-20 px-h-100-40 overflow-auto">
      ...
      <nav className="px-fs-14 px-lh-32">
        ...
        <a className="theme-tx0 block" href="#path">
          {_('path()')}
        </a>
        <a className="theme-tx0 block" href="#set">
          {_('set()')}
        </a>
        <a className="theme-tx0 block" href="#with-args">
          {_('withArgs()')}
        </a>
        <a className="theme-tx0 block" href="#with-form-data">
          {_('withFormData()')}
        </a>
        <a className="theme-tx0 block" href="#with-path">
          {_('withPath()')}
        </a>
        <a className="theme-tx0 block" href="#with-query">
          {_('withQuery()')}
        </a>
      </nav>
    </menu>
...
export function Body() {
  return (
    <article className="px-h-100-0 overflow-auto px-px-20 px-pb-20 px-fs-15">
      ...
      <P>
        A callable nest is a type of callable class. In this case it's 
        a <H>Nest()</H> class that can be called as a function. 
        A <H>Nest()</H> is a class made to easily manage nested objects.
      ...
      <section>
        ...
        <Code>{`data.data.foo.foo //--> 'bar'`}</Code>
      </section>
      ...
      <section>
        ...
        <Code>{`get<T extends UnknownNest = M>(): T;`}</Code>
        <Code>{`get<T = any>(...path: Key[]): T`}</Code>
        <Code>{`get<T = any>(...path: Key[]): UnknownNest|T`}</Code>

        <H4>Example</H4>

        <Code>{`data.get() //--> { foo: { foo: 'bar', bar: 'foo' }, bar: 'foo' }`}</Code>
        <Code>{`data.get('foo') //-->  { foo: 'bar', bar: 'foo' }`}</Code>
        <Code>{`data.get('foo', 'foo') //--> 'bar'`}</Code>
        <Code>{`data.get<string>('foo', 'foo') //--> 'bar'`}</Code>
        <Code>{`data.get<{ foo: string, bar: string }>('foo')`}</Code>
      </section>
      ...
      <section>
        ...
        <Code>{`path<T = any>(path: string, defaults?: TypeOf<T>): TypeOf<T>`}</Code>

        <H4>Example</H4>

        <Code>{`data.path('foo.foo')`}</Code>
        <Code>{`data.path<string>('foo.foo')`}</Code>
        <Code>{`data.path('foo.foo', 'barzoo')`}</Code>
      </section>
      ...
      <section>
        ...
        <Code>{`data.set('foo', 'foo', 'bar')`}</Code>
      </section>
      ...
      <section>
        <P>
          Nest extension that parses CLI arguments and sets them
          in the nest. 
        </P>

        <Code>{`data.withArgs.set('-v --name john age=30') // { v: true, name: 'john', age: 30 }`}</Code>
      </section>

      {/*------------------------------------------------------------*/}
                        
      <a id="with-form-data"></a>
      <H2>withFormData()</H2>

      <section>
        <P>
          Nest extension that parses form data and sets them
          in the nest. 
        </P>

        <Code>{`data.withFormData.set(req.body)`}</Code>
      </section>
      ...
      <section>
        ...
        <Code>{`data.withPath.get('foo.foo')`}</Code>
        <Code>{`data.withPath.get('foo-foo', '-')`}</Code>

        <Code>{`data.withPath.delete('foo.foo')`}</Code>
        <Code>{`data.withPath.delete('foo-foo', '-')`}</Code>

        <Code>{`data.withPath.has('foo.foo')`}</Code>
        <Code>{`data.withPath.has('foo-foo', '-')`}</Code>

        <Code>{`data.withPath.set('foo.foo', 'bar')`}</Code>
        <Code>{`data.withPath.set('foo-foo', 'bar', '-')`}</Code>
      </section>
      ...
      <section>
        <P>
          Nest extension that parses query data and sets them
          in the nest.
        </P>

        <Code>{`data.withQuery.set('foo=bar&bar=foo')`}</Code>
      </section>
      ...
      <Nav
        prev={{ 
          text: 'Callable Map', 
          href: '/docs/references/data/callable-map' 
        }}
        next={{ 
          text: 'Callable Session', 
          href: '/docs/references/data/callable-session' 
        }}
      />
    </article>
  );
