...
//docs
import { H1, H2, P, H, A, C } from '../../../components/index.js';
import { Nav, Code, Layout } from '../../../components/index.js';

const examples = [
//0-------------------------------------------------------------------//
`import { session } from 'stackpress/lib'
const data = session()
const data = session([['foo', 'bar'], ['bar', 'foo']])`,
//1-------------------------------------------------------------------//
];
...
export function Head(props: ServerPageProps<ServerConfigProps>) {
  ...
  const { _ } = useLanguage();
  //variables
  const title = _('Callable Session - Data - Stackpress Documentation');
  const description = _(
    'A callable session is a type of callable map used to manage '
    + 'session data.'
  );
  return (
    <>
  ...
  return (
    <menu className="px-m-0 px-px-10 px-py-20 px-h-100-40 overflow-auto">
      ...
      <nav className="px-fs-14 px-lh-32">
        ...
        <a className="theme-tx0 block" href="#revisions">
          {_('revisions')}
        </a>
...
export function Body() {
  return (
    <article className="px-h-100-0 overflow-auto px-px-20 px-pb-20 px-fs-15">
      <H1>Callable Session</H1>

      <P>
        A callable session is a type 
        of <A href="/docs/references/data/callable-map">callable map</A>. 
      ...
      <P>
        Once created you can use it as a function. The callable session
      ...
      <section>
        <P>
          Returns the revisions object. The revisions object is a map
          that contains the name and 
          revision <C>{'{ action, value }'}</C> where action is 
          either <C>'set'</C> or <C>'remove'</C>. The
          revisions object is used to track the changes made to
          the session in order to update the cookies.
        </P>

        <Code>{`data.revisions.name.action`}</Code>
      </section>
      ...
      <section>
        <P>
          Returns the number of key/value pairs in the session. This is
      ...
      <section>
        <P>
          Clears the session. This will remove all the keys and values
          from the session.
        </P>
      ...
      <section>
        <P>
          Deletes the key/value pair from the session. If the key does
      ...
      <section>
        <P>
          ...
          pairs for each element in the session. 
        </P>
      ...
      <section>
        <P>
          Calls a function for each key/value pair in the session. The
