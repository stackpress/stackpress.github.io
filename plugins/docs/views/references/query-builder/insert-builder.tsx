...
export function Head(props: ServerPageProps<ServerConfigProps>) {
  ...
  const { _ } = useLanguage();
  //variables
  const title = _('Insert Builder - Query Builder - References - Stackpress Documentation');
  const description = _(
  ...
  return (
    <menu className="px-m-0 px-px-10 px-py-20 px-h-100-40 overflow-auto">
      ...
      <nav className="px-fs-14 px-lh-32">
        ...
        <a className="theme-tx0 block" href="#returning">
          {_('returning()')}
        </a>
...
export function Body() {
  return (
    <article className="px-h-100-0 overflow-auto px-px-20 px-pb-20 px-fs-15">
      <H1>Insert Query Builder</H1>

      <P>
        Query builder for the SQL <H>INSERT</H> statement.
      </P>
      ...
      <section>
        ...
        <Code>{`db.insert.engine //same as db`}</Code>
        <Code>{`db.insert.engine = db`}</Code>
      </section>
      ...
      <section>
        ...
        <Code>{`const { table, returning, values } = db.insert('user').build()`}</Code>
      </section>
      ...
      <section>
        ...
        <Code>{`const queries = db.insert('user').query()`}</Code>
      </section>
      ...
      <section>
        ...
        <Code>{`returning(columns: string|string[] = '*'): InsertBuilder`}</Code>
        
        <H4>Example</H4>
        
        <Code>{`db.insert('user').returning()`}</Code>
        <Code>{`db.insert('user').returning('id')`}</Code>
        <Code>{`db.insert('user').returning([ 'id', 'name' ])`}</Code>
      </section>
      ...
      <section>
        <P>
          Sets the values to be inserted. This can be a single object
          or an array of objects.
        </P>
        
        <H4>Usage</H4>
        
        <Code>{`values(values: Record<string, Value>|Record<string, Value>[])`}</Code>
        
        <H4>Example</H4>
        
        <Code>{`db.insert('user').values({ name: 'John' })`}</Code>
        <Code>{`db.insert('user').values([ { name: 'John' }, { name: 'Jane' } ])`}</Code>
      </section>
